...
switch(dir){
	...
	case "abajo":	
		sprite_index = spr_soldado_atacar_abajo;
		if (animacion_ataque) instance_create_depth(x,y+64,-0,obj_soldadoattack);
		break;
	case "arriba":
		sprite_index = spr_soldado_atacar_arriba;
		if (animacion_ataque) instance_create_depth(x,y-64,0,obj_soldadoattack);
		break;
	case "izquierda":
		sprite_index = spr_soldado_atacar_izquierda;
		if (animacion_ataque) instance_create_depth(x-64,y+8,0,obj_soldadoattack);
		break;
	case "derecha":
		sprite_index = spr_soldado_atacar_derecha;
		if (animacion_ataque) instance_create_depth(x+64,y+8,0,obj_soldadoattack);
		break;
